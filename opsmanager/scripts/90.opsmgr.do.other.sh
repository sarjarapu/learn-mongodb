...
sudo -u mongod sh -c 'echo  | openssl sha1 -sha512  | sed 's/(stdin)= //g' > /data/keyfile'
...
use admin 
db.createUser({user: 'mms-automation', pwd: '', roles: ['clusterAdmin', 'dbAdminAnyDatabase', 'readWriteAnyDatabase', 'restore', 'userAdminAnyDatabase']})
